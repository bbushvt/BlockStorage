...
3. Create a file system on the new partition.
   ...
   - The new partition should be listed with the disk, similar to XXXlp1, followed by the size, Type (83), and Linux. Remember the partition name; you’ll need it in Step 3.
   - mkfs.ext3 /dev/mapper/XXXlp1 (where XXXlp1represents the partition name is that was returned in Step 2).
4. Create a mount point for the file system and mount it.
   - mkdir /PerfDisk (or where you want to mount the file system).
   - mount /dev/mapper/XXXlp1 /PerfDisk (using the partition name from Step 2).
...
<table border="0" cellpadding="0" cellspacing="0">
		<tbody>
			...
			<tr>
				<td>
						<li>
							<code>Command action: p</code>
						</li>
				</td>
				<td>Makes the partition the primary one.
				</td>
			</tr>
			<tr>
				<td>
				...
				<td>Becomes partition 1 on the disk.
				</td>
			</tr>
			<tr>
				<td>
						<li>
							<code>First cylinder (1-8877): 1 (default)</code>
						</li>
				</td>
				<td>Start at cylinder 1.
				</td>
			</tr>
			<tr>
				<td>
						<li>
							<code>Last cylinder, +cylinders or +size {K, M, G}: 8877 (default)</code>
						</li>
				</td>
				<td>Hit Enter to go to the last cylinder.
				</td>
			</tr>
			<tr>
				<td colspan="2">
			...
			<tr>
				...
				<td>Selects partition 1 to be set up as a specific type.
				</td>
			</tr>
			<tr>
				<td>
...
Disk identifier: 0x0009170d 
Disk /dev/sdc: 21.5 GB, 21474836480 bytes 
Disk identifier: 0x2b5072d1 
Disk /dev/sdb: 21.5 GB, 21474836480 bytes 
Disk identifier: 0x2b5072d1 
Disk /dev/mapper/3600a09803830304f3124457a45757066: 21.5 GB, 21474836480 bytes 
Disk identifier: 0x2b5072d1
```
...
```
root@server:~# multipath -l -v 3 | grep sd 
Feb 17 19:55:02 | sda: device node name blacklisted 
Feb 17 19:55:02 | sdb: device node name blacklisted 
Feb 17 19:55:02 | sdc: device node name blacklisted 
Feb 17 19:55:02 | sdd: device node name blacklisted 
Feb 17 19:55:02 | sde: device node name blacklisted
```
...
root@server:~# fdisk -l | grep Disk 
Disk /dev/sda: 500.1 GB, 500107862016 bytes 
Disk identifier: 0x0009170d Disk /dev/sdc: 21.5 GB, 21474836480 bytes 
Disk identifier: 0x2b5072d1 Disk /dev/sdb: 21.5 GB, 21474836480 bytes 
Disk identifier: 0x2b5072d1
```
